...
report_json_file = REPORT_PATH + "\\classification_report_base_rf.json"
report_confusion_matrix = REPORT_PATH + "\\confusion_matrix_base_rf.json"
...
"""
def predict_evaluate_base_rf():

    dagshub.init(repo_owner='se4ai2324-uniba', repo_name='MalURLs', mlflow=True)
    mlflow.autolog()
    mlflow.start_run()

    model = joblib.load(MODEL_PATH+"\\base_rf_model.pkl")
